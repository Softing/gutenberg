...
sub create {
    ...
    unless (@{ $result->{errors} }) {
        $path = $c->Q(" SELECT path FROM catalog WHERE id =? ",
        [ $i_path ])->Value;
    }
...
sub update {
    ...
    unless (@{ $result->{errors} }) {
        $count = $c->Do(" UPDATE catalog SET title=?, shortcut=?, description=?, path=replace(?, '-',  '')::ltree WHERE id=? ",
...
sub delete {
    ...
    $c->Do(" DELETE FROM catalog WHERE id =? ", [ $id ]);
