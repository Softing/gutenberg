...
sub read {
    ...
    unless (@errors) {
        $result = $c->sql->Q("
            SELECT id, edition, title, shortcut, description, bydefault, w, h, created, updated FROM ad_pages WHERE id=?
        ", [ $i_id ])->Hash;
    }
...
sub list {
    ...
    my @params;
    my $sql = " SELECT id, edition, title, shortcut, description, bydefault, w, h, created, updated FROM ad_pages WHERE edition=? ";
...
sub create {
    ...
    unless (@errors) {
        $c->sql->Do("
            INSERT INTO ad_pages(id, edition, title, shortcut, description, bydefault, w, h, created, updated)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, now(), now());
        ", [ $id, $edition->{id}, $i_title, $i_shortcut, $i_description, $default, \@i_w, \@i_h ]);
    }
...
sub update {
    ...
    unless (@errors) {
        $c->sql->Do("
            UPDATE ad_pages SET title=?, shortcut=?, description=?, bydefault=?, w=?, h=?, updated=now() WHERE id = ? ;
        ", [ $i_title, $i_shortcut, $i_description, $default, \@i_w, \@i_h, $page->{id} ]);
    }
