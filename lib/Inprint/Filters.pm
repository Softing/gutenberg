...
sub mysession
{
    ...
    if ($sid) {
        ...
        my $session = $c->sql->Q("SELECT * FROM inp_session WHERE sid=?", [ $sid ])->Hash;
        
        if ($session) {
            
            $c->sql->Do(" UPDATE passport.session SET updated=now() WHERE sid=? ", [ $sid ] );
            
            $c->session("login", $session->{member_login});
            $c->session("title", $session->{member_title});
            $c->session("stitle", $session->{member_stitle});
