...
sub read {
    ...
    unless (@errors) {
        $result = $c->sql->Q("
            SELECT id, fascicle, title, shortcut, description, w, h, created, updated FROM fascicles_tmpl_pages WHERE id=?
        ", [ $i_id ])->Hash;
    }
...
sub list {
    ...
    my @params;
    my $sql = " SELECT id, fascicle, title, shortcut, description, w, h, created, updated FROM fascicles_tmpl_pages WHERE fascicle=? ";
...
sub create {
    ...
    unless (@errors) {
        $c->sql->Do("
            INSERT INTO fascicles_tmpl_pages(id, fascicle, origin, title, shortcut, description, w, h, created, updated)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, now(), now());
        ", [ $id, $fascicle->{id}, $fascicle->{id}, $i_title, $i_shortcut, $i_description, \@i_w, \@i_h ]);
    }
...
sub update {
    ...
    unless (@errors) {
        $c->sql->Do("
            UPDATE fascicles_tmpl_pages SET title=?, shortcut=?, description=?, w=?, h=?, updated=now() WHERE id = ? ;
